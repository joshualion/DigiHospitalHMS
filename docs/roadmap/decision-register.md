...
| Phase 1B.1 public theme system | Approved and implemented: CSS-token theme system with light/dark/system appearance, four controlled accents, visitor persistence, and admin-published defaults | Improves public UI quality without replacing the CMS or accepting arbitrary CSS | Approved for Phase 1B.1 | Hospital admin/marketing input |
| Phase 1B.1 icon package | Approved: use `@lucide/vue` as the single Vue-compatible icon package for public UI controls | Provides tree-shakable Vue icons without multiple overlapping icon libraries | Approved for Phase 1B.1 | No |
