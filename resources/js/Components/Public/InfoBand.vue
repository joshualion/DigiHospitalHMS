...
<template>
    <section v-if="items.length" class="relative z-10 -mt-16 px-4 sm:px-6 lg:px-8">
        <div class="public-container relative">
            ...
            <div class="relative grid overflow-hidden rounded-[2rem] border shadow-2xl md:grid-cols-3" style="background: var(--public-surface-elevated); border-color: var(--public-border); box-shadow: var(--public-shadow);">
                <article v-for="item in items" :key="item.heading" class="group border-b p-6 text-center transition hover:-translate-y-0.5 md:border-b-0 md:border-r lg:p-8" style="border-color: var(--public-border);">
                    ...
                    <Link v-if="item.link_label" :href="item.url || '/contact'" class="public-focus public-link mt-4 inline-flex text-sm font-black">{{ item.link_label }}</Link>
                </article>
            </div>
        </div>
    </section>
