...
function saveItem() {
    item.post('/admin/inventory/items', { preserveScroll: true, onSuccess: () => { activeModal.value = null; item.reset(); } });
}

function saveLocation() {
    location.post('/admin/inventory/locations', { preserveScroll: true, onSuccess: () => { activeModal.value = null; location.reset(); } });
}

function saveUnit() {
    unit.post('/admin/inventory/units', { preserveScroll: true, onSuccess: () => { activeModal.value = null; unit.reset(); } });
}
</script>
...
<template>
    ...
    <AppLayout title="Inventory Catalogue">
        <PageHeader title="Inventory Catalogue" description="Medicine, non-medicine items, units and inventory locations.">
            <template #actions>
                <ActionToolbar align="end">
                    ...
                    <PrimaryButton type="button" @click="activeModal = 'item'">Add Item</PrimaryButton>
                    <PrimaryButton type="button" @click="activeModal = 'location'">Add Location</PrimaryButton>
        ...
        <div class="grid gap-6">
            <section class="min-w-0 rounded-lg border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="mt-5 divide-y divide-slate-100 dark:divide-slate-800">
                    <article v-for="entry in items" :key="entry.id" class="min-w-0 py-3">
                        <p class="break-words font-bold">{{ entry.sku }} - {{ entry.name }}</p>
                        <p class="break-words text-sm text-slate-500">{{ entry.type }} - {{ entry.base_unit?.code }} - reorder {{ entry.reorder_level }}</p>
                    </article>
                    <p v-if="items.length === 0" class="py-4 text-sm text-slate-500">No items configured.</p>
                </div>
            </section>

            <section class="grid gap-6 lg:grid-cols-2">
                <div class="min-w-0 rounded-lg border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                    <h2 class="font-black">Locations</h2>
                    <p v-for="entry in locations" :key="entry.id" class="mt-3 break-words text-sm">{{ entry.code }} - {{ entry.name }} - {{ entry.type }}</p>
                    <p v-if="locations.length === 0" class="mt-3 text-sm text-slate-500">No locations configured.</p>
                </div>
                <div class="min-w-0 rounded-lg border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                    <h2 class="font-black">Units</h2>
                    <p v-for="entry in units" :key="entry.id" class="mt-3 break-words text-sm">{{ entry.code }} - {{ entry.name }}</p>
                    <p v-if="units.length === 0" class="mt-3 text-sm text-slate-500">No units configured.</p>
                </div>
            </section>
        </div>

        <FormModal :show="activeModal === 'item'" title="Add Item" :form="item" submit-label="Create item" size="full" @close="activeModal = null" @submit="saveItem">
        ...
        <FormModal :show="activeModal === 'location'" title="Add Location" :form="location" submit-label="Create location" @close="activeModal = null" @submit="saveLocation">
        ...
        <FormModal :show="activeModal === 'unit'" title="Add Unit" :form="unit" submit-label="Create unit" @close="activeModal = null" @submit="saveUnit">
            <div class="grid gap-3">
