...
function saveRequest() {
    form.post('/admin/laboratory/requests', { preserveScroll: true, onSuccess: () => { showOrder.value = false; form.reset(); } });
}
</script>
...
<template>
    ...
    <AppLayout title="Laboratory Requests">
        <PageHeader title="Laboratory Requests" description="Laboratory order worklist and request entry.">
            <template #actions>
                <ActionToolbar align="end">
                    <Link href="/admin/laboratory/catalogue" class="rounded-md border px-3 py-2 text-sm font-bold">Catalogue</Link>
                    <PrimaryButton type="button" @click="showOrder = true">Order Lab Request</PrimaryButton>
                </ActionToolbar>
            </template>
        </PageHeader>
        ...
        <section class="min-w-0 rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
                <Link v-for="item in requests.data" :key="item.id" class="grid min-w-0 gap-3 p-4 md:grid-cols-[1fr_auto]" :href="`/admin/laboratory/requests/${item.id}`">
                    <div class="min-w-0">
                        <p class="break-words font-bold">{{ item.request_number }} - {{ item.patient?.full_name }}</p>
                        <p class="break-words text-sm text-slate-500">{{ item.accession_number }} - {{ item.tests.map((test) => test.test_name).join(', ') }}</p>
                    </div>
        ...
        <FormModal :show="showOrder" title="Order Lab Request" :form="form" submit-label="Order request" size="full" @close="showOrder = false" @submit="saveRequest">
            <div class="grid gap-3 md:grid-cols-2">
                ...
                <select v-model="form.lab_test_ids" class="rounded-md border-slate-300 md:col-span-2" multiple><option v-for="test in labTests" :key="test.id" :value="test.id">{{ test.code }} - {{ test.name }}</option></select>
                <select v-model="form.lab_test_profile_ids" class="rounded-md border-slate-300 md:col-span-2" multiple><option v-for="profile in labProfiles" :key="profile.id" :value="profile.id">{{ profile.code }} - {{ profile.name }}</option></select>
                <textarea v-model="form.clinical_notes" class="rounded-md border-slate-300 md:col-span-2" rows="3" placeholder="Clinical notes for the lab"></textarea>
            </div>
        </FormModal>
    </AppLayout>
</template>
