...
function submit() {
    form.post('/admin/patients', {
        preserveScroll: true,
        onSuccess: () => {
            if (duplicateWarnings.value.length === 0) form.reset();
        },
    });
}
</script>
        ...
        <div class="grid gap-6 xl:grid-cols-[1fr_430px]">
            <section class="rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                <div class="grid gap-3 border-b border-slate-200 p-4 md:grid-cols-[1fr_180px] dark:border-slate-800">
                    <input v-model="search.search" class="rounded-md border-slate-300 dark:border-slate-700 dark:bg-slate-900" placeholder="Search hospital number, name, phone or identifier" @change="filter">
                ...
                <div class="divide-y divide-slate-100 dark:divide-slate-800">
                    <Link v-for="patient in patients.data" :key="patient.id" :href="`/admin/patients/${patient.id}`" class="grid gap-2 p-4 hover:bg-slate-50 md:grid-cols-[1fr_160px_120px] dark:hover:bg-slate-800">
                        <div>
                            <p class="font-bold">{{ patient.full_name }}</p>
                            <p class="text-sm text-slate-500">{{ patient.hospital_number }} · {{ patient.sex }} · {{ patient.phone || 'No phone' }}</p>
                        </div>
                        <p class="text-sm text-slate-600">{{ patient.registration_facility?.name }}</p>
                        <span class="w-fit rounded-full px-2 py-1 text-xs font-bold" :class="patient.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'">{{ patient.status }}</span>
                    </Link>
                    <p v-if="patients.data.length === 0" class="p-4 text-sm text-slate-500">No patients found.</p>
                </div>
            </section>

            <form class="rounded-lg border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900" @submit.prevent="submit">
                <h2 class="text-lg font-bold">Register Patient</h2>

                <div v-if="duplicateWarnings.length" class="mt-4 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
                ...
                <div v-if="validationErrors.length" class="mt-4 rounded-md border border-rose-300 bg-rose-50 p-3 text-sm text-rose-900">
                ...
                <div class="mt-5 space-y-3">
                    <div class="flex items-center justify-between"><h3 class="font-bold">Identifiers</h3><button class="rounded-md border px-2 py-1 text-sm" type="button" @click="addIdentifier">Add</button></div>
                    <div v-for="(identifier, index) in form.identifiers" :key="index" class="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                        ...
                        <button class="rounded-md border border-rose-300 px-2 py-1 text-sm text-rose-700" type="button" @click="removeIdentifier(index)">Remove</button>
                    </div>
                </div>

                <div class="mt-5 space-y-3">
                    <div class="flex items-center justify-between"><h3 class="font-bold">Contacts and Next of Kin</h3><button class="rounded-md border px-2 py-1 text-sm" type="button" @click="addContact">Add</button></div>
                    <div v-for="(contact, index) in form.contacts" :key="index" class="grid gap-2 rounded-md border border-slate-200 p-3">
                        ...
                        <button class="w-fit rounded-md border border-rose-300 px-2 py-1 text-sm text-rose-700" type="button" @click="removeContact(index)">Remove contact</button>
                    </div>
                </div>

                <PrimaryButton class="mt-5" :disabled="form.processing">Register patient</PrimaryButton>
            </form>
        </div>
    </AppLayout>
</template>
